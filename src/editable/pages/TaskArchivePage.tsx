...
const taskDeck: Record<TaskKey, { icon: typeof FileText; archiveClass: string; promise: string; badge: string }> = {
  mediaDistribution: { icon: Newspaper, archiveClass: 'grid gap-5 md:grid-cols-2 xl:grid-cols-3', promise: 'Campaign cards prioritize source, category, headline, reach, status, and publication-ready summaries.', badge: 'News' },
...
export async function EditableTaskArchiveRoute({
  ...
  const result = await fetchPaginatedTaskPosts(task, { page, limit: 24, category })
  let posts = result.posts
  let pagination = result.pagination

  if (!posts.length && page === 1) {
    const fallbackPosts = await fetchTaskPosts(task, 24, { allowMockFallback: true, fresh: true })
    posts = category === 'all'
      ? fallbackPosts
      : fallbackPosts.filter((post) => normalizeCategory(getCategory(post, '')) === category)
    pagination = {
      page: 1,
      limit: 24,
      total: posts.length,
      ...
      <main className="min-h-screen bg-[var(--slot4-page-bg)] text-[var(--slot4-page-text)]">
        <section className="bg-[var(--slot4-body-gradient)]">
          <div className="mx-auto flex max-w-[var(--editable-container)] flex-col gap-6 px-4 py-10 sm:px-6 lg:flex-row lg:items-end lg:justify-between lg:px-8 lg:py-14">
            <div>
              <p className="text-xs font-black uppercase tracking-[0.28em] text-[var(--slot4-accent)]">Browsing media distribution</p>
              <h1 className="mt-3 text-5xl font-black leading-none tracking-[-0.035em] sm:text-6xl lg:text-7xl">
                {category === 'all' ? label : categoryLabel}
              </h1>
            </div>
            <p className="max-w-md rounded-2xl border border-black/10 bg-white/80 p-5 text-sm font-bold leading-7 text-black/65 shadow-sm">
              Search publisher-ready releases, brand announcements, campaign assets, and distribution signals organized for fast media review.
            </p>
          </div>
        </section>

        <section className="border-y border-black/10 bg-[var(--slot4-dark-bg)] text-white">
        ...
        {lead ? (
          <section className="mx-auto grid max-w-[var(--editable-container)] gap-6 px-4 py-8 sm:px-6 lg:grid-cols-[1.65fr_0.8fr] lg:px-8">
            <Link href={`${basePath}/${lead.slug}`} className="group relative min-h-[34rem] overflow-hidden rounded-[2rem] bg-[var(--slot4-dark-bg)] shadow-[0_24px_70px_rgba(16,24,39,.16)]">
              ...
              <div className="absolute inset-x-0 bottom-0 p-6 text-white sm:p-9">
                ...
                <h2 className="mt-5 max-w-4xl text-4xl font-black leading-[0.98] tracking-[-0.035em] sm:text-6xl">{lead.title}</h2>
                <p className="mt-5 max-w-2xl line-clamp-2 text-sm font-semibold leading-7 text-white/80">{getSummary(lead)}</p>
              </div>
            </Link>
            <div className="grid overflow-hidden rounded-[2rem] border border-black/10 bg-white shadow-[0_16px_48px_rgba(16,24,39,.08)]">
              <div className="border-b border-black/10 bg-[var(--slot4-accent-soft)] p-6 text-[var(--slot4-page-text)]">
                <p className="text-xs font-black uppercase tracking-[0.24em]">Top stories</p>
                <p className="mt-3 text-3xl font-black leading-tight">Campaigns gaining publisher attention.</p>
              </div>
        ...
        <section className="mx-auto max-w-[var(--editable-container)] px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
          <div className="mb-8 flex flex-wrap items-end justify-between gap-5 border-b border-black/10 pb-4">
            <h2 className="text-4xl font-black tracking-[-0.035em] sm:text-5xl">More campaigns</h2>
          ...
          {remaining.length ? (
            <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
              {remaining.map((post, index) => (
                <Link key={post.id || post.slug} href={`${basePath}/${post.slug}`} className="group overflow-hidden rounded-[1.5rem] border border-black/10 bg-white shadow-[0_12px_36px_rgba(16,24,39,.07)] transition hover:-translate-y-1 hover:shadow-[0_20px_58px_rgba(16,24,39,.13)]">
            ...
            <div className="rounded-[1.5rem] border border-dashed border-black/20 bg-white p-12 text-center">
          ...
          <div className="mt-10 flex items-center justify-center gap-0">
            {pagination.hasPrevPage ? <Link href={pageHref(basePath, category, page - 1)} className="rounded-full border border-black/10 bg-white px-5 py-3 text-xs font-black uppercase">Previous</Link> : null}
            ...
            {pagination.hasNextPage ? <Link href={pageHref(basePath, category, page + 1)} className="rounded-full border border-black/10 bg-white px-5 py-3 text-xs font-black uppercase">Next</Link> : null}
          </div>
        </section>
      </main>
    </EditableSiteShell>
